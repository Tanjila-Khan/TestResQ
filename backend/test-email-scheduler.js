...
async function testAPIEndpoints() {
  ...
  try {
    ...
    const schedulerResponse = await axios.get(`${BASE_URL}/api/scheduler/status`);
    console.log('✅ Scheduler status endpoint working:', schedulerResponse.data);
    
    // Test email queue status endpoint
    const queueResponse = await axios.get(`${BASE_URL}/api/email/queue-status`);
    console.log('✅ Email queue status endpoint working:', queueResponse.data);
...
async function testManualEmailScheduling() {
  console.log('\n4️⃣ Testing Manual Email Scheduling...');
